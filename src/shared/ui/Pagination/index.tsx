      ...
      {pages.map((page, i) => {
        ...
        return (
          <div key={page} className="flex items-center gap-2">
            {!noGap && '...'}
            {page === currentPage ? (
              <span className="w-10 h-10 rounded-md bg-amber-400 flex items-center justify-center text-black text-xs font-extrabold">
                {page}{' '}
              </span>
            ) : (
              <Link
                href={`?page=${page}`}
                className="w-10 h-10 rounded-md bg-cyan-400 hover:bg-cyan-300 flex items-center justify-center text-black text-xs font-extrabold"
              >
                {page}
              </Link>
            )}
